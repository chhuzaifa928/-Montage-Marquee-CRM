...
export default async function BookingVoucherPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen bg-slate-100/80 py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="no-print max-w-4xl mx-auto mb-6 flex items-center justify-between">
        <Link
          href="/"
          className="inline-flex items-center space-x-2 text-xs font-semibold text-slate-600 hover:text-slate-900 bg-white px-3.5 py-2 rounded-xl shadow-xs border border-slate-200"
        >
          <ArrowLeft className="w-4 h-4" />
          <span>Back to Operations Suite</span>
        </Link>

        {/* Client Print trigger button */}
        <div className="flex items-center space-x-3">
          <Link
            href="/"
            className="text-xs text-amber-700 font-semibold"
          >
            Montage Operations Suite
          </Link>
        </div>
      </div>

      {/* Main Official Printable Document */}
      <div className="printable-voucher max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-200 p-6 sm:p-12 text-slate-900">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs my-6">
        ...
        {/* Bank Credentials */}
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs my-6">
          <span className="font-bold text-slate-700 uppercase tracking-wider block mb-1">
        ...
        {/* Legal Signatures */}
        <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-end gap-6 text-[11px] text-slate-700">
